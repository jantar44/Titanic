...
# Widać nieznaczny wpływ widoczny dla osób posiadających 3 rodziców/dzieci - można odrzucić <br><br><br>
...
# Największe szanse przeżycia miały rodziny 3 osobowe. <br><br><br>
...
# Widać znaczny wpływ klasy na przeżywalność. <br><br><br>
...
data.drop(['Ticket', 'Cabin','rozmiar_rodziny'],axis=1,inplace=True)
...
# Uzupełnienie brakujących danych średnią (30 lat).
...
# Istnieje możliwość dokładniejszego odwzorowania brakujących danych - średnie wieku dla poszczególnych klas, płci były różne
...
data['Fare'].fillna(33.30,inplace=True)
...
nauka_S.shape, nauka_D.shape, wynik_D.shape
...
coeff_df = pd.DataFrame(nauka_D.columns.delete(0))
coeff_df.columns = ['Feature']
coeff_df["Correlation"] = pd.Series(reglog.coef_[0])
...
Y_pred = decision_tree.predict(wynik_D)
...
random_forest.fit(nauka_D, nauka_S)
Y_pred1 = random_forest.predict(wynik_D)
random_forest.score(nauka_D, nauka_S)
acc_random_forest = round(random_forest.score(nauka_D, nauka_S) * 100, 2)
acc_random_forest
